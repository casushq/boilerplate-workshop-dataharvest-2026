...
# Naming schema
#
# Pattern: food_{key}.parquet, resolved inside the data directory
# (see _default_data_dir below).
#
#   food.parquet        — full dataset (~7.5 GB)
#   food_eu.parquet     — EU27 + UK combined
#   food_us.parquet     — United States
#   food_fr.parquet     — France
#   food_de.parquet     — Germany
#   food_nl.parquet     — Netherlands
#   food_es.parquet     — Spain
#   food_it.parquet     — Italy
#   food_be.parquet     — Belgium
#   food_pl.parquet     — Poland
#   food_se.parquet     — Sweden
#   food_dk.parquet     — Denmark
# ---------------------------------------------------------------------------

# Maps short key → filename (resolved inside the data directory)
...
#
# Resolution order, so the skill works both on a provisioned workshop VM
# (data pre-staged by the orchestrator) and in local development:
#   1. $OFF_DATA_DIR                       — explicit override
#   2. ~/data/openfoodfacts/  (if it exists) — workshop-VM standard location
#   3. ./data                              — local / dev fallback
def _default_data_dir() -> str:
    """Resolve the parquet data directory (see resolution order above)."""
    env = os.environ.get("OFF_DATA_DIR")
    if env:
        return env
    vm_dir = Path.home() / "data" / "openfoodfacts"
    if vm_dir.is_dir():
        return str(vm_dir)
    return "data"
...
class OFFParquet:
    ...
    def download(
        ...
        Example:
            OFFParquet.download(f"{DEFAULT_DATA_DIR}/food.parquet")
        """
        try:
            import requests
    ...
    def build_subset(
        ...
        Uses the naming schema food_{key}.parquet, written into data_dir.
        ...
        Args:
            ...
            data_dir: Output directory. Default: DEFAULT_DATA_DIR.

        Returns:
            Path to the created subset file.

        Example:
            OFFParquet.build_subset("nl")          # → <data_dir>/food_nl.parquet
            OFFParquet.build_subset("eu")          # → <data_dir>/food_eu.parquet
        """
