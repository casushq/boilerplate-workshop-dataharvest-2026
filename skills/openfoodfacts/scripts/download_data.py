...
Files land in the data directory resolved by off_parquet.DEFAULT_DATA_DIR
($OFF_DATA_DIR, else ~/data/openfoodfacts/ on a VM, else ./data) — or
wherever --data-dir points. Exits non-zero if the download is incomplete
or any subset fails to build, so callers never mistake a partial run for
a complete one.
...
Expected output structure (inside the resolved data directory):
...
def download_full(data_dir: str = DEFAULT_DATA_DIR) -> Path:
    ...
    The download streams to a ``.tmp`` file that is renamed onto the final
    path only after the byte count matches the server's content-length.
    An interrupted run therefore never leaves a truncated food.parquet
    that a later run would mistake for a complete download and skip.
    """
    ...
    tmp.unlink(missing_ok=True)  # discard any partial file from an interrupted run
    ...
    try:
        ...
        if total and tmp.stat().st_size != total:
            raise IOError(
                f"incomplete download: got {tmp.stat().st_size} bytes, "
                f"expected {total}"
            )
    ...
    tmp.replace(dest)  # atomic publish — dest exists only once complete
...
def build_subsets(
    ...
    Returns the list of keys that failed to build (empty on full success),
    so the caller can exit non-zero rather than silently reporting success.
    """
...
def print_schema() -> None:
    ...
    print("=" * 60)
    print(f"  {'Key':<6}  {'File':<26}  Country / region")
    print("-" * 60)
...
Examples:
  ...
  python scripts/download_data.py --status           # show which files exist
    ...
    parser.add_argument(
        "--skip-download",
        action="store_true",
        help="Skip downloading food.parquet (assumes it already exists).",
    )
    parser.add_argument(
        "--data-dir",
    ...
    failed = build_subsets(keys, data_dir=args.data_dir)

    # Step 3: final status
    print_status(args.data_dir)
    if failed:
        print(
            f"✗ Setup incomplete — {len(failed)} subset(s) failed: "
            f"{', '.join(failed)}"
        )
        sys.exit(1)
