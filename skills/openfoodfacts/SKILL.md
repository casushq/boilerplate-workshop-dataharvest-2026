...
On workshop VMs the parquet files are **pre-staged** by the orchestrator at
`~/data/openfoodfacts/`. You do not download anything — `OFFParquet`
resolves that location automatically. Always load data by its short key
(below); never guess a path.

If a key's file is missing, the background download is still running — see
[If the data isn't there yet](#if-the-data-isnt-there-yet). Do **not**
download it yourself.

Pattern: `food_{key}.parquet`

| Key | File | Contents | Approx. size |
|---|---|---|---|
| `full` | `food.parquet` | Full dataset | ~7.5 GB |
| `eu` | `food_eu.parquet` | EU27 + UK combined | ~1.5 GB |
| `us` | `food_us.parquet` | United States | ~0.5 GB |
| `fr` | `food_fr.parquet` | France | ~0.8 GB |
| `de` | `food_de.parquet` | Germany | ~0.3 GB |
| `gb` | `food_gb.parquet` | United Kingdom | ~0.3 GB |
| `es` | `food_es.parquet` | Spain | ~0.2 GB |
| `it` | `food_it.parquet` | Italy | ~0.2 GB |
| `nl` | `food_nl.parquet` | Netherlands | ~0.15 GB |
| `be` | `food_be.parquet` | Belgium | ~0.1 GB |
| `pl` | `food_pl.parquet` | Poland | ~0.1 GB |
| `se` | `food_se.parquet` | Sweden | ~0.1 GB |
| `dk` | `food_dk.parquet` | Denmark | ~0.05 GB |
| `at` `fi` `pt` `ro` `hu` `cz` `ie` `gr` | `food_{key}.parquet` | Mid-size EU countries | ~0.05–0.1 GB |
| `sk` `hr` `bg` `si` `lt` `lv` `ee` `lu` `cy` `mt` | `food_{key}.parquet` | Smaller EU countries | <0.05 GB |
...
On a workshop VM the data is **pre-staged** at `~/data/openfoodfacts/` and
`OFFParquet` finds it automatically. You normally never touch the
filesystem — just load by key:

```python
import sys
sys.path.insert(0, "/path/to/skill/scripts")  # adjust to the skill location
from off_parquet import OFFParquet

off = OFFParquet.from_key("eu")     # resolves ~/data/openfoodfacts/food_eu.parquet
```

`OFFParquet` resolves the data directory in this order: `$OFF_DATA_DIR`,
then `~/data/openfoodfacts/`, then `./data/`.

#### If the data isn't there yet

If `from_key()` raises `FileNotFoundError`, the orchestrator's background
download has not finished. **Do not download it yourself** — that
re-fetches 7.5 GB to the wrong place and wastes the participant's session.
Instead, check on the background job:

```bash
systemctl status off-data-prep.service --no-pager   # is it still running?
tail -n 20 ~/data/openfoodfacts/prepare.log         # download / build progress
ls ~/data/openfoodfacts/.done                       # appears only when fully ready
```

Tell the user the dataset is still being prepared and roughly how far
along it is, then wait for `.done` rather than working around it.

### Step 1b: Provisioning the data (NOT during a workshop session)

This is run once when the VM image is built — you should not need it
during a session. It downloads `food.parquet` and builds the country
subsets into the resolved data directory, and **exits non-zero if the
download is incomplete or any subset fails** (a clean exit means the data
is genuinely ready):
...
python scripts/download_data.py --list                       # naming schema
